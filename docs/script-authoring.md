...
This guide is for someone providing a Python workload to a Home Platform
operator. A workload currently consists of one Python file and one CSV input.
The operator may submit both through Job Desk or the CLI and choose an eligible
compute worker.
...
The author should give the operator every required input up front. If another
dependency is needed, it must be reviewed and added to a future image rather
than installed dynamically by the script.
...
available only after successful completion. A future progress protocol will be
added separately.
